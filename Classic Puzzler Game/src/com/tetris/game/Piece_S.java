...
/**
 *
 * Class that represent the S piece of the tetris's game
 *              
 *          ____ ____           ____                               ____
 *     FREE|b2  |b1  |     FREE|b1  | FREE     FREE FREE FREE     |b1  | FREE FREE
 *     ____|____|____|         |____|____           ____ ____     |____|____
 *    |b4  |b3+ |FREE      FREE|b2+ |b3  |     FREE|b2+ |b1  |    |b2  |b3+ | FREE
 *    |____|____|              |____|____|     ____|____|____|    |____|____|
 *     FREE FREE FREE      FREE FREE|b4  |    |b4  |b3  | FREE     FREE|b4  | FREE
 *                                  |____|    |____|____|              |____|
 *         
 *         
...
public class Piece_S extends Piece {
	...
	public static final String colorS = "#00FF00";			// green
	public static final String marginColorS = "#000000";	// black
	
	/**
	 * 
	 * Constructs a Piece_S object with the specified arguments. This object represent the S piece of tetris's game.
	 * It uses the default colors: green of filler and black for margin.
	 * 
	 * @param b1 Block 1
	 * @param b2 Block 2
	 * @param b3 Block 3
	 ...
	 */
	public Piece_S(Block b1, Block b2, Block b3, Block b4, Tetris board) {
		super(b1, b2, b3, b4, colorS, marginColorS, board);
	}
	 ...
	 * @param board Tetris's game
	 */
	public Piece_S(Block b1, Block b2, Block b3, Block b4, Tetris board, String color, String marginColor) {
	...
	public synchronized boolean rotate() {
		...
		if (rotation == EnumPieceRotation.Rot0Degrees) {
			...
			if(!board.isBlockBusy(b3.getRow()-1, b3.getColumn()-1) && !board.isBlockBusy(b3.getRow(), b3.getColumn()+1) && !board.isBlockBusy(b3.getRow()+1, b3.getColumn()+1)){
				b2.freeBlock();
				b2 = b3;
				b1.freeBlock();
				b4.freeBlock();
			...
			if(!board.isBlockBusy(b2.getRow()-1, b2.getColumn()+1) && !board.isBlockBusy(b2.getRow()+1, b2.getColumn()) && !board.isBlockBusy(b2.getRow()+1, b2.getColumn()-1)){
				...
				generateShadow();
				return true;
			} else if(!board.isBlockBusy(b2.getRow()-1, b2.getColumn()+1) && !board.isBlockBusy(b2.getRow()+1, b2.getColumn()) && !board.isBlockBusy(b2.getRow(), b2.getColumn()+2)){
			...
			if(!board.isBlockBusy(b2.getRow()+1, b2.getColumn()+1) && !board.isBlockBusy(b2.getRow(), b2.getColumn()-1) && !board.isBlockBusy(b2.getRow()-1, b2.getColumn()-1)){
				b3.freeBlock();
				b3 = b2;
				b1.freeBlock();
				b4.freeBlock();
			...
			if(!board.isBlockBusy(b3.getRow()+1, b3.getColumn()-1) && !board.isBlockBusy(b3.getRow()-1, b3.getColumn()) && !board.isBlockBusy(b3.getRow()-1, b3.getColumn()+1)){
				// Space free in both sides
				...
				generateShadow();
				return true;
			} else if(!board.isBlockBusy(b3.getRow()+1, b3.getColumn()-1) && !board.isBlockBusy(b3.getRow()-1, b3.getColumn()) && !board.isBlockBusy(b3.getRow(), b3.getColumn()-2)){
